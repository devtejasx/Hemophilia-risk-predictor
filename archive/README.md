...
| `legacy-ml/` | `train.py`, `data_fusion.py`, `evaluation.py`, `predict.py`, `shap_explainability.py`, `explainers.py`, `ensemble_models.py`, `imbalance_handler.py`, `train_optimized.py` | The synthetic-data training pipeline. Preserved because it documents exactly how `ml/artifacts/legacy-synthetic-v0/` was produced. `explainers.py` is the generic `SHAPExplainer` / `LIMEExplainer` / `ExplainabilityComparison` trio that `ml/explainability/service.py` grew out of; nothing imported it. `ensemble_models.py` and `imbalance_handler.py` came from `ml/models/`, where nothing in the active tree imported them — `train.py` here is their only caller, and its bare `from ensemble_models import ...` now resolves from this directory. The run's scratch output (`catboost_info/`, `temp_roc_curve.png`, `temp_report.json`, two empty metric CSVs) was removed: it is regenerable by re-running these scripts, and it described the 24-train/6-test fabricated split, so no number in it may be quoted. The code is the provenance record, not its output. |
