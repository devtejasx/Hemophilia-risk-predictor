...
| `legacy-ml/` | `train.py`, `data_fusion.py`, `evaluation.py`, `predict.py`, `shap_explainability.py`, `explainers.py`, `train_optimized.py`, `catboost_info/`, `temp_*` | The synthetic-data training pipeline. Preserved because it documents exactly how `ml/artifacts/legacy-synthetic-v0/` was produced. `explainers.py` is the generic `SHAPExplainer` / `LIMEExplainer` / `ExplainabilityComparison` trio that `ml/explainability/service.py` grew out of; nothing imported it. |
