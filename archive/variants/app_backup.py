...
Professional multi-page Streamlit app with modular architecture
"""
...
st.set_page_config(
    ...
    menu_items={
        "About": "Clinical Intelligence & Risk Assessment System v3.0",
...
# Import custom modules (improved error handling)
custom_modules = {
    'database': None,
    'gpt_chatbot': None,
    'user_auth': None,
    'evaluation': None,
    'clinical_assistant': None,
}
...
try:
    ...
    custom_modules['database'] = True
except ImportError as e:
    logger.warning(f"Database module not fully available: {e}")
...
try:
    ...
    custom_modules['gpt_chatbot'] = True
except ImportError as e:
    logger.warning(f"GPT Chatbot module not available: {e}")

try:
    from user_auth import UserManager
    custom_modules['user_auth'] = True
except ImportError as e:
    logger.warning(f"User auth module not available: {e}")

try:
    from evaluation import ModelEvaluator
    custom_modules['evaluation'] = True
except ImportError as e:
...
try:
    ...
    custom_modules['clinical_assistant'] = True
except ImportError as e:
    logger.warning(f"Clinical assistant module not available: {e}")

# Try to import SHAP explainability - with fallback
try:
    ...
    logger.info("SHAP explainability module not available - will use fallback")

# ============= DATABASE INITIALIZATION =============
if custom_modules['database']:
    try:
        init_database()
        if custom_modules['user_auth']:
    ...
    logger.warning("⚠️ Database module not available - app will run in demo mode")

# ============= CACHED MODEL LOADING (with proper cache management) =============
@st.cache_resource(show_spinner=False)
def load_models():
    """Load trained models with memory-efficient loading"""
    ...
    try:
        ...
        if os.path.exists("rf.pkl"):
            try:
                ...
                logger.warning(f"⚠️ Could not load rf.pkl: {str(e)[:50]}")
        ...
        if os.path.exists("xgb.pkl"):
            try:
                ...
                logger.warning(f"⚠️ Could not load xgb.pkl: {str(e)[:50]}")
        
        if os.path.exists("columns.pkl"):
            try:
                columns = joblib.load("columns.pkl", mmap_mode='r')
                logger.info("✅ Column metadata loaded")
            except (MemoryError, EOFError, pickle.UnpicklingError) as e:
                logger.warning(f"⚠️ Cache corrupted, retrying without mmap: {str(e)[:30]}")
                try:
                    columns = joblib.load("columns.pkl")
                except Exception as retry_e:
                    logger.warning(f"⚠️ Could not load columns.pkl: {str(retry_e)[:50]}")
        
        return rf_model, xgb_model, columns
    except MemoryError:
        logger.error("⚠️ Memory Error: Unable to load models")
        return None, None, None
    except Exception as e:
        logger.error(f"⚠️ Error loading models: {str(e)[:100]}")
        return None, None, None

# Cache busting helper
@st.cache_resource
def get_cache_version():
    """Returns a version token to help bust cache when needed"""
...
<style>
    * {
    ...
    /* Apply animations */
    [data-testid="stExpander"], .card {
        animation: fadeIn 0.6s ease-out, slideInLeft 0.8s ease-out;
    }
    
    [data-testid="stMetric"] {
        animation: fadeIn 0.5s ease-out, countUp 0.6s ease-out;
    }
...
# User is authenticated - Continue to main app
