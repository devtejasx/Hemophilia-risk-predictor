...
retired CHAMP pipeline and was deleted with it; its conclusions survive as the
table below.
...
| **Dataset** | `ml/data/BVTH_VTH-2024-000215-mmc2.csv` + `…-mmc3.csv`, read only through `ml/preprocessing/hemophilia_a.py` | the CHAMP variant-level dataset (deleted, not archived) | MMC3 is a clinical-record table joinable to the MMC2 mutation description, which gives the model clinical assay values alongside the genomic block. CHAMP was variant-level only and carried no assay values. MMC3's records are aggregated per `mut_id`, so a modelling row is one mutation. |
| **Preprocessing** | `ml/preprocessing/hemophilia_a.py` — one fitted `ColumnTransformer` per feature set | the CHAMP preprocessor (deleted), `data_fusion.py` | `data_fusion` imputed over the full dataset before splitting, and shipped a duplicate and a constant feature. The CHAMP preprocessor was correct for its own variant-level column set but could not express the MMC2/MMC3 feature blocks, the per-`mut_id` aggregation, or the grouped split. |
...
| **SHAP / LIME** | `ml/explainability/service.py` | `ml/explainability/explainers.py`, `shap_explainability.py`, `backend/services/explainability.py`, `pages/shap_explainability*.py` | `service.py` is what the API and tests call: one service, both methods, reporting against source columns rather than encoded names. `explainers.py` held the generic explainer classes it grew out of; it was on no call path and has been moved to `archive/legacy-ml/`. |
...
| **Logging** | `logging.basicConfig` in `backend/main.py`, level from `LOG_LEVEL` | `backend/core/logging.py`, scattered `print()` calls | One configuration call at startup, and every module uses `logging.getLogger(__name__)`. `backend/core/logging.py` was a JSON-formatter module from the pre-consolidation backend: nothing imported it and it could not be imported (`from config import settings` raises `ModuleNotFoundError`). Archived under `archive/legacy-backends/`. |
...
The CHAMP artifact (`champ-v1`) was deleted with its pipeline, so
`available_versions()` cannot list it and the API cannot serve it. Its metrics
described a different, variant-level dataset and must not be quoted for these
models.

## Database

`case_records` replaced the CHAMP-era `genomic_profiles` table. That table's
fixed variant-level columns (`variant_type`, `mechanism`, `domain`, …) could not
hold an MMC2/MMC3 record, and a fixed-column replacement would need migrating
every time the feature resolution changes — so the validated feature dict is
stored as JSON alongside its feature set. The migration that used to rename
CHAMP-era tables aside has been removed: the schema in `backend/db.py` is the
only one the application creates or reads.
